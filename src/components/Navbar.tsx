  ...
  return (
    <>
      <motion.header
        ...
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 px-4 sm:px-8 md:px-12 py-4 ${
          scrolled ? 'py-3' : 'py-5'
        }`}
      >
        <div
          className={`max-w-7xl mx-auto rounded-full border border-white/10 transition-all duration-300 px-5 sm:px-7 py-3 flex items-center justify-between ${
            scrolled
              ? 'bg-[#0E0E14]/85 backdrop-blur-md shadow-[0_10px_30px_rgba(0,0,0,0.8)]'
              : 'bg-[#08080A]/60 backdrop-blur-sm'
          }`}
        >
          {/* Brand Logo & Name */}
          <a href="#" className="flex items-center gap-3 group">
            <div className="relative w-9 h-9 rounded-full overflow-hidden border border-volt/50 p-0.5 bg-black">
              <img
                src="/assets/icon.png"
                alt="NOKA AXL Logo"
            ...
            <div className="flex flex-col">
              <span className="font-kanit font-black text-lg sm:text-xl tracking-wider text-white group-hover:text-volt transition-colors flex items-center gap-1.5">
                NOKA AXL
                <span className="inline-block w-1.5 h-1.5 rounded-full bg-volt animate-ping" />
              </span>
              <span className="text-[9px] font-mono tracking-widest text-slate-400 uppercase -mt-1 hidden sm:block">
                DJ // PRODUCER
              </span>
            </div>
          </a>
          ...
          <div className="flex items-center gap-3 sm:gap-4">
            ...
            >
              {isPlayingAudio ? (
                <>
                  ...
                  <span className="hidden sm:inline">LIVE SYNTH</span>
            ...
            <Magnet padding={40} strength={4}>
              <button
                ...
                className="relative group overflow-hidden px-4 sm:px-6 py-2 rounded-full bg-volt text-black font-kanit font-bold text-xs sm:text-sm uppercase tracking-wider transition-all duration-300 hover:shadow-volt-md active:scale-95 flex items-center gap-1.5"
              >
                <Calendar className="w-3.5 h-3.5" />
                <span>BOOK ARTIST</span>
                <Sparkles className="w-3.5 h-3.5 text-black/70 group-hover:rotate-45 transition-transform" />
              </button>
            </Magnet>
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div
            ...
            className="fixed inset-x-4 top-20 z-40 bg-[#0E0E14]/95 border border-white/10 backdrop-blur-xl rounded-3xl p-6 shadow-2xl lg:hidden"
          >
