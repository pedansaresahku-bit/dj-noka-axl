...
class DatabaseService {
  ...
  public createEvent(data: Omit<CalendarEvent, 'id'>): CalendarEvent {
    ...
    const newEvent: CalendarEvent = {
      ...data,
      id: `cal-${data.day}-${Date.now().toString().slice(-4)}`,
      createdAt: new Date().toISOString(),
      updatedAt: new Date().toISOString(),
    };

    // Remove any existing event on the same day if desired, or allow multiple
    const filtered = events.filter(e => e.day !== newEvent.day);
  ...
  // Bookings CRUD
  public getAllBookings(): BookingInquiry[] {
    this.ensureDataFiles();
    try {
  ...
  public createBooking(data: Omit<BookingInquiry, 'id' | 'status' | 'createdAt'>): BookingInquiry {
  ...
  public updateBookingStatus(id: string, status: BookingInquiry['status']): boolean {
    const bookings = this.getAllBookings();
