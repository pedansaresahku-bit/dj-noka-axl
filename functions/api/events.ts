...
export async function onRequestGet(context: any) {
  ...
  try {
    if (env?.EVENTS_KV) {
      ...
      if (stored && Array.isArray(stored)) {
        return new Response(JSON.stringify({ success: true, count: stored.length, data: stored }), {
  ...
  return new Response(JSON.stringify({ success: true, count: DEFAULT_EVENTS.length, data: DEFAULT_EVENTS }), {
    headers: { 'Content-Type': 'application/json' },
  });
}
...
export async function onRequestPost(context: any) {
  ...
  try {
    ...
    const newEvent = {
      ...body,
      id: `cal-${body.day}-${Date.now().toString().slice(-4)}`,
      createdAt: new Date().toISOString(),
    };

    const filtered = events.filter((e: any) => e.day !== newEvent.day);
    filtered.push(newEvent);
    filtered.sort((a: any, b: any) => a.day - b.day);

    if (env?.EVENTS_KV) {
      await env.EVENTS_KV.put('events_data', JSON.stringify(filtered));
    }

    return new Response(JSON.stringify({ success: true, message: 'Event created', data: newEvent }), {
      headers: { 'Content-Type': 'application/json' },
    });
  } catch (err: any) {
